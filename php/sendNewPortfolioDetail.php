...
error_reporting(E_ALL);*/
    ...
    if ($_SERVER['REQUEST_METHOD']=="POST") {
        ...
        $upload_dir ="../NodeJS/public/PortfolioImages/";
        ...
        if (file_exists($upload_dir)) {
            if (is_writable($upload_dir)) {
                ...
                header("location: ../add-portfolio.php?success=0");
            }
        } else {
            header("location: ../add-portfolio.php?success=0");
        }

        $url = $config['URLS']['API_PATH'] . $config['URLS']['ADD_PORTFOLIO'];
        ...
        if ($res->status=="ok") {        
            header("location: ../add-portfolio.php?success=1");   
        }
        else {
            header("location: ../add-portfolio.php?success=0");
        }
